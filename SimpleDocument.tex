...
constraint with sign ’=’, and 5 variables, consider the dual problem D:
\begin{enumerate}[label=\Alph*]
...
\item | is used to solve NP-complete (“difficult”) problems; (d)  terminates in a polynomial number of iterations;
...
\begin{enumerate}[label=\Alph*]
\item | a collection of $m$ constraints; (b) none of the above answers.; 
\item |  a square $m$x$m$ matrix with value 1 on the main diagonal; \item |  a collection of $n-m$ columns of the constraint matrix; \item |  a square submatrix of the constraint matrix, that can be inverted.
\end{enumerate}
}%
}%
\\
