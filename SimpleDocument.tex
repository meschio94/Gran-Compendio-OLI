...
\usepackage{graphicx} %pacchetti per mettere immagini in equazioni
\usepackage[export]{adjustbox}

\usepackage{hyperref} %per gli hyperlink

\usepackage{enumitem}%per numerare gli item con le lettere, usato nella sezione 6
...
&min: \Delta\\
    	...
    	&\displaystyle \sum_{i = 1}^{70} x_{ig} = 7 & &g=\{1,\dots,10\}\\
...
&max: z= \displaystyle \sum_{i = 1}^{m1} \sum_{j = 1}^{n} x_{ij}f_j\\
    	...
    	&\displaystyle \sum_{j=1}^{n} l_{j}x_{ij} \leqq L1 & &i=1,\dots,m1 \\
    	&\displaystyle \sum_{j=1}^{m} l_{j}x_{ij} \leqq L2 & &i=m1+1,\dots,m \\
...
\raisebox{-.5\height}{\includegraphics[width=4cm]{immagini/ProblemaDuale1.jpg}}
\end{center}
...
\item | there are more variables
than constraints;
\item | the number of basic variables with zero value is equal to the number of
constraints;
\item | the number of basic variables with zero value is equal to the difference of the
number of columns and rows (n − m);
\item | the number of basic variables with non-zero value is
equal to the number of rows in the constrains matrix;
\item | the basic matrix is invertible.
\end{enumerate}
}%
}%
\\
...
\item | defines the minimum and maximum value of
b which does not change the optimal basis;
\item | defines the minimum and maximum value of b
which does not change the value of the optimal solution;
\item | defines the range of values of b
which does not change the reduced costs;
\item | defines the range of values of b which does not
...
\noindent\fbox{%
 ...
 \begin{enumerate}[label=\Alph*]
\item | is used to find the computational complexity of the
problem;
\item | can be applied to minimization problems in standard form; \item | finds the optimal
solution of an NP-complete problem;
\item | always use as bounding procedure the solution of a
PLC problem;
\item | none of the above answers.
\end{enumerate}
}%
}%
\\
...
\noindent\fbox{%
    \parbox{\textwidth}{%
Given a PLC problem P in minimization form, with 2 constraints with sign '$\geqq$' and 1 constraint with sign '=', and 5 variables, consider the dual problem D:
\begin{enumerate}[label=\Alph*]
\item | D has 3 variables
and 5 constraints;
\item | D has 5 variables and 3 constraints;
\item | the first variable of D must be
non-negative;
\item | the third variable of D is unrestricted in sign;
\item | the second variable of D
...
\begin{enumerate}[label=\Alph*]
\item | is used to solve linear programming problems with continuous variables;
\item | terminates in a polynomial number of iterations;
\item | is used to solve
NP-complete (”difficult”) problems;
\item | at each iteration uses the simplex algorithm to solve
the current subproblem;
\item | uses the lowest-first strategy to explore the subproblems.
\end{enumerate}
}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | if the primal has finite solution, then the dual has finite solution;
\item | a feasible dual solution may have a value smaller than that of the optimal primal solution;
\item | if the primal is unbounded, then the dual is unbounded;
\item | any feasible solution of the primal has a value greater or equal to any feasible solution of the dual;
\item | if the primal is empty, then the dual is empty.
\end{enumerate}
}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | cannot be used in a graph with negative cost arcs;
\item | cannot be used in a graph with negative circuits;
\item | can be used to find the shortest path in a directed graph, from a source node to a sink node;
\item | can be used to find the minimum cost spanning tree;
\item | cannot be applied to acyclic graphs.
\end{enumerate}
}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | uses a decision tree with an exponential number of levels; \item | uses a decision tree with a polynomial number of levels; \item | uses the lowest-first strategy to explore the decision tree; 
\item | computes an upper bound at each node of the decision tree; \item | uses a decision tree with an exponential number of nodes.
\end{enumerate}
}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | is used to solve linear programming problems with continuous variables; 
\item | always uses the lowest-first strategy to explore the subproblems.;
\item | is used to solve NP-complete (“difficult”) problems; 
\item | terminates in a polynomial number of iterations;
\item | at each iteration uses the simplex algorithm to solve the current subproblem.
\end{enumerate}
}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | a collection of $m$ constraints;
\item | none of the above answers.; 
\item |  a square $m$x$m$ matrix with value 1 on the main diagonal; 
\item |  a collection of $n-m$ columns of the constraint matrix; 
\item |  a square submatrix of the constraint matrix, that can be inverted.
\end{enumerate}
}%
}%
\\
...
\subsection{Soluzione Base}
\noindent\fbox{%
    \parbox{\textwidth}{%
Consider the two problems, $P:min\{c^Tx:Ax = b, x \geqq 0\}$, $D: max\{u^T: u^T A \leqq c^T\}$, then: 
\begin{enumerate}[label=\Alph*]
\item | a pair of solution $x$ and $u$ with $x$ feasible for $P$ and $u$ feasible for $D$ are optimal if $\{Ax-b\}u^T=0$; 
\item | a pair of solution $x$ and $u$ with $x$ feasible for $P$ and $u$ feasible for $D$ are optimal if $c^T x \geqq u^T b $; 
\item | a feasible solution of $P$ gives a lower bound on the optimal value of $D$; 
\item | a solution $u$ is optimal if $u^TA \leqq c^T$ and $u^T \geqq 0$; 
\item | a feasible solution of $D$ gives a lower bound on the optimal value of $P$.
\end{enumerate}
	}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | at each iteration computes the shortest path from the source to the sink; 
\item | terminate when an $s-t$ cut of the graph is saturated; 
\item | uses saturated arcs in their opposite direction; 
\item | minimizes the toal cost of the flow; 
\item | uses non-empty, non-saturated arcs in both directions.
\end{enumerate}
	}%
}%
\\
...
\subsection{PLI minimization \& relaxation}
\noindent\fbox{%
    \parbox{\textwidth}{%
...
\begin{enumerate}[label=\Alph*]
\item | the least integer greater than or equal to $z*$ is a lower bound on the optimal $IP$ value if the coefficents of the objective function are integer; 
\item | $z*$ is a lower bound on the optimal $IP$ value; 
\item | the least integer greater than or equal to $z*$ is an upper bound on the optimal $IP$ value; 
\item | $z*$ is an upper bound on the optimal $IP$ value; 
\item | the least integer greater than or equal to $z*$ is a lower bound on the optimal $IP$ value if the coefficents of the objective function are rational numbers;
\end{enumerate}
	}%
}%
\\
...
\subsection{B\&B knapsack 0-1}
\noindent\fbox{%
    \parbox{\textwidth}{%
The branch-and-bound method for 0-1 knapsack problem: 
\begin{enumerate}[label=\Alph*]
\item | stop the search when level $n$ is reached; 
\item | computes the upper bound only in the son nodes generated by a constraint of the kind $x=1$; 
\item | computes the upper bound only in the son nodes generated by a constraint of the kind $x=0$; 
\item | use the highest first strategy to explore the tree; 
\item | has $O(2^n)$ tree levels;
\end{enumerate}
	}%
}%
\\
...
\begin{enumerate}[label=\Alph*]
\item | can be solved exactly using the simplex algorithm with the addition of Gomory cuts; 
\item | can be solved exactly using the simplex algorithm with a non-polynomial number of iteration; 
\item | cannot be solved exactly in polynomial time; 
\item | can be solved exactly using a binary decision-tree with $n$ levels, where $n$ is the number of variables; 
\item | can be solved exactly using Dijkstra algorithm;
\end{enumerate}
	}%
}%
\\
